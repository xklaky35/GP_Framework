...
namespace Engine {
    ...
    AssetBrowser::AssetBrowser(bool visible) {
    ...
    struct AssetField {
        ...
        AssetField(int id, int type, bool is_selected, Vector2d position) : ID(id), Type(type), IsSelected(is_selected), position(position) {}
        ...
        static int IMGUI_CDECL CompareWithSortSpecs(const void* lhs, const void* rhs) {
            const AssetField* a = (const AssetField*)lhs;
            const AssetField* b = (const AssetField*)rhs;
    ...
    void AssetBrowser::AddItems(int count)
    {
        if (Items.Size == 0)
            NextItemId = 0;
        Items.reserve(Items.Size + count);
        for (int n = 0; n < count; n++, NextItemId++)
            Items.push_back(AssetField(NextItemId, 0, false, Vector2d{0,0}));
        RequestSort = true;
    }
