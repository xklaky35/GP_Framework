...
namespace Engine {
    ...
    SceneEditor::SceneEditor(bool visible) {
    ...
    struct SelectableField {
        ...
        SelectableField(int id, int type, bool is_selected, Vector2d position) : ID(id), Type(type), IsSelected(is_selected), position(position) {}

        static const ImGuiTableSortSpecs* s_current_sort_specs;

        static void SortWithSortSpecs(ImGuiTableSortSpecs* sort_specs, SelectableField* items, int items_count) {
        ...
        static int IMGUI_CDECL CompareWithSortSpecs(const void* lhs, const void* rhs) {
            const SelectableField* a = (const SelectableField*)lhs;
            const SelectableField* b = (const SelectableField*)rhs;
    ...
    const ImGuiTableSortSpecs* SelectableField::s_current_sort_specs = nullptr;

    void SceneEditor::AddItems(int count)
    {
        if (Items.Size == 0)
            NextItemId = 0;
        Items.reserve(Items.Size + count);
        for (int n = 0; n < count; n++, NextItemId++)
            Items.push_back(SelectableField(NextItemId, 0, false, Vector2d{0,0}));
        RequestSort = true;
    }
    void SceneEditor::ClearItems()
    {
        Items.clear();
        Selection.Clear();
    }
    ...
    void SceneEditor::UpdateLayoutSizes(float avail_width)
    {
    ...
    void SceneEditor::Draw(const char* title, bool* p_open)
    {
        ...
        if (AllowSorting)
        {
            ...
            if (ImGui::BeginTable("for_sort_specs_only", 2, table_flags_for_sort_specs, ImVec2(0.0f, ImGui::GetFrameHeight())))
            {
                ...
                if (ImGuiTableSortSpecs* sort_specs = ImGui::TableGetSortSpecs())
                    ...
                    {
                        SelectableField::SortWithSortSpecs(sort_specs, Items.Data, Items.Size);
        ...
        if (ImGui::BeginChild("Assets", ImVec2(0.0f, -ImGui::GetTextLineHeightWithSpacing()), ImGuiChildFlags_Borders, ImGuiWindowFlags_NoMove))
        {
            ...
            while (clipper.Step())
            {
                for (int line_idx = clipper.DisplayStart; line_idx < clipper.DisplayEnd; line_idx++)
                {
                    ...
                    {
                        SelectableField* item_data = &Items[item_idx];
